...
def extract_season_episode(input_text, rename_mode):
    """Extract season and episode numbers from filename or caption"""
    for pattern, (season_group, episode_group) in METADATA_PATTERNS:
        match = pattern.search(input_text)
        if match:
            season = match.group(1) if season_group == 'season' else None
            episode = match.group(2) if episode_group == 'episode' else (match.group(1) if episode_group == 'episode' else None)
...
def extract_quality(input_text, rename_mode):
    """Extract quality information from filename or caption"""
...
async def add_metadata(input_path, output_path, user_id):
    ...
    cmd = [
        ffmpeg,
        '-i', input_path,
        '-metadata', f'title={metadata["title"] or ""}',
        '-metadata', f'artist={metadata["artist"] or ""}',
        '-metadata', f'author={metadata["author"] or ""}',
        '-metadata:s:v', f'title={metadata["video_title"] or ""}',
        '-metadata:s:a', f'title={metadata["audio_title"] or ""}',
        '-metadata:s:s', f'title={metadata["subtitle"] or ""}',
        '-map', '0',
        '-c', 'copy',
        '-loglevel', 'error',
        output_path
...
async def handle_extraction(client, message):
    """Handler for files sent after selecting extraction method"""
    user_id = message.from_user.id
    choice = await codeflixbots.get_user_choice(user_id)
    
    if not choice:
        return await auto_rename_files(client, message)  # Fallback to default rename if no extraction choice
    ...
    try:
        # Determine input text based on user choice
        input_text = file_name if choice == "filename" else (message.caption or file_name)
        rename_mode = choice
        ...
        try:
            ...
            await msg.delete()
            # Reset user choice after successful processing
            await codeflixbots.set_user_choice(user_id, None)
        except Exception as e:
...
async def auto_rename_files(client, message):
    ...
    try:
        ...
        quality = extract_quality(file_name, "filename")
